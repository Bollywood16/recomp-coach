...
- **Event-sourced sync (sessions/weights/settings tables)**: implemented on branch `event-sourced-sync-wip`, held back from `main` pending schema verification on the fitness Supabase project (`hxufyphmfkhjcetlwhgx`) — an earlier migration attempt ran against the wrong project (`anzbpxqvibgpxnwgyqoc`, the forecasting app) by Codespace mixup, so table existence there is unconfirmed. `cloudPush` is additive-only (insert-if-missing, one row per session/weigh-in — no more whole-blob overwrite); `cloudPushBlob` dual-writes to the old `app_data` table as a revert switch until the new path is proven. Deploy plan: verify/create the three tables + RLS in the fitness project, backfill from `app_data`, verify row counts match, only then merge to `main`. Full plan in `docs/bucket-b-plan.md`.
